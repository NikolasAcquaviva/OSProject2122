...
pcb_t *allocPcb(){
	...
	if(list_empty(&pcbFree_h)) return NULL;
		...
		//prendo il primo pcb libero (campo next perché usiamo sentinella)
		...
		tmp->p_semAdd = NULL;
		tmp->p_time = 0;
		//ricordiamoci di rimuovere il pcb allocato dalla lista dei pcb liberi
		list_del(&tmp->p_list);
		return tmp;
		}
}
...
coda dei processi puntata da head*/
	...
	//inserisce il pcb puntato da p in coda
...
*/
	...
	//aggiorniamo il parent del nuovo figlio
	p->p_parent = prnt;
}
...
*/
	...
	//caso in cui p non ha figli
	if (list_empty(&p->p_child)) return NULL;
	else{
		pcb_t *tmp = container_of(p->p_child.next, pcb_t, p_list);
		// tmp = pcb primo figlio
		//se li ha, caso in cui il figlio non ha fratelli
		if (list_empty(&tmp->p_sib)){
			list_del(&tmp->p_list);
			return tmp;
		}
		//caso in cui il primo figlio ha fratelli
		//bisogna aggiornare il primo figlio di p
		//che sarà il primo fratello del precedente primo figlio(che ora sarà rimosso)
		else{
			...
			tmp_child->p_parent = tmp_sib; //il figlio del pcb da rimuovere deve aggiornare il parent(che non sarà piu il pcb rimosso)
			pcb_t * iter;
...
*/
	...
	//caso in cui p è il primo figlio del suo genitore (usiamo funzione che rimuove il primo figlio)
