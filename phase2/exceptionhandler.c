...
static void PassUp_Or_Die(int index){
    if(currentProcess->p_supportStruct == NULL){
		...
		//LDCXT allows a current process to change its operating mode/context - non carichiamo uno stato pieno di registri come con LDST e.g. non nuovo stato ma cambio contesto/mode con timeslice non influenzato
...
void SYSCALLExceptionHandler(){
    ...
    if(a0 <= -1 && a0 >= -10 && user == 1){
        /* non bastava fare exceptState->cause |= (EXC_RI << CAUSE_EXCCODE_BIT); ? */
...
static void Die (pcb_t *p, int isRoot){
    ...
    if (p->p_semAdd != NULL){
        ...
        /* else if(*p->p_semAdd == 0 && headBlocked(p->p_semAdd) == NULL) *p->p_semAdd = 1; //il processo si era accaparrato la risorsa => il prossimo processo che la richiederà la otterrà subito. */
            ...
            if(*p->p_semAdd == 0 && headBlocked(p->p_semAdd) == NULL) *p->p_semAdd = 1; //il processo si era accaparrato la risorsa => il prossimo processo che la richiederà la otterrà subito.
        }
...
static pcb_PTR FindProcess(int pid){
    ...
    //il caso running è escluso, la funzione viene chiamata dalla nsys2/ già gestito implicitamente
    ...
    //controllo su tutti i pcb su tutti i semafori
...
void TERM_PROCESS(int pid, int a2, int a3){
    ...
    if(pid == 0){ //"se il secondo parametro è 0 il bersaglio è il processo invocante SLIDE PRES FASE 2"

        if(emptyChild(currentProcess) == 0) {
            RecursiveDie(container_of(currentProcess->p_child.next,pcb_t,p_sib)); //eliminiamo prima i suoi figli
            Die(currentProcess,1); //poi ci suicidiamo
        }
    }
    else{
...
void DO_IO(int *cmdAddr, int cmdValue, int a3){
...
int GET_PROCESS_ID(int parent, int a2, int a3){
    ...
    else if (parent == 0) return currentProcess->p_pid;
}
